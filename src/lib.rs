...
//! let mut query_client = QueryClient::new(Client::new(base_url.to_string())); // `QueryClient` is the client generated by tonic
//!
...
//! ## Streaming requests
//!
//! The request body is sent in one piece: for client-streaming and bidirectional calls, the client collects the whole
//! request stream before it sends the request. Such a call starts only when its request stream ends, so a bidirectional
//! call whose next request waits for a response never starts.
mod abort_guard;
mod body_stream;
mod call;
mod client;
